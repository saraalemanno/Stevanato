...
def start_encoder_simulation(device):
    ...
    try:
        while encoder_running:
            ...
            if ready2go:
                isRunningPin = 0b100000000                          # DIO8 isRunning pin high
            elif not ready2go:
                isRunningPin = 0b000000000                          # DIO8 isRunning pin low
...
def get_main_status(URL_API):
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\033[1m\033[91mERROR\033[0m fetching main status: {response.status_code}")
            return None
