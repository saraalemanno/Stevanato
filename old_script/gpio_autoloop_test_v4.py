...
# Date: 2025-08-07
# Version: 4
# Delta from previous version: Removed the Main module addition, managed in a separate script.
...
def get_device_address():
    if len(sys.argv) >= 3:
        ...
        try:
            ...
            print(f"Error: {e}")
            sys.exit(1)
        return device_namespace, device_name, deviceType, NPin_list
...
def run_gpio_autoloop_test(device_namespace):
    ...
    def connect():
        print("Connecting to device with address ", device_address)
    ...
    def connect():
        ...
        print("Manually added device ", device_name, " with address: ", device_address)
        time.sleep(2)                                      
