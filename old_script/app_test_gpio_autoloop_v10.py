...
# Date: 2025-08-20
# Version: 10 to use with gpio_autoloop_test_v5 and I2C_test_v1 and encoder_test and add_noise_v1
# Delta from previous versions: Route to add noise changed.
    ...
    <script>
        ...
        function addGalvoDevice() {
            ...
            block.innerHTML = `
                ...
                <div class="loop-fields" style="display:none;">
                    <label>Begin:</label><br><input type="number" name="begin"><br>
                    <label>End:</label><br><input type="number" name="end"><br>
                    <label>Time Up:</label><br><input type="number" name="time_up"><br>
                    <label>Time Down:</label><br><input type="number" name="time_down"><br>
                    <label>Delay:</label><br><input type="number" name="delay"><br>
                    <label>Cycles:</label><br><input type="number" name="cycles"><br>
                    <label>Command active:</label><br>
                    <select name="cmd">
            ...
            const modeSelect = block.querySelector('select[name="mode"]');
            const manualFields = block.querySelector('.manual-fields');
            const loopFields = block.querySelector('.loop-fields');

            modeSelect.addEventListener('change', () => {
                if (modeSelect.value === 'manual') {
                    manualFields.style.display = 'block';
                    loopFields.style.display = 'none';
                } else {
                    manualFields.style.display = 'none';
                    loopFields.style.display = 'block';
                }
            });
            ...
            runButtonGalvo.addEventListener('click', async() => {
                const address = block.querySelector('input[name="address"]').value;
                const mode = block.querySelector('select[name="mode"]').value;

                let payload = { address, mode };

                if (mode === 'manual') {
                    const angle = block.querySelector('input[name="angle"]').value;
                    payload.angle = angle;
                } else {
                    payload.begin = block.querySelector('input[name="begin"]').value;
                    payload.end = block.querySelector('input[name="end"]').value;
                    payload.time_up = block.querySelector('input[name="time_up"]').value;
                    payload.time_down = block.querySelector('input[name="time_down"]').value;
                    payload.delay = block.querySelector('input[name="delay"]').value;
                    payload.cycles = block.querySelector('input[name="cycles"]').value;
                    payload.cmd = block.querySelector('select[name="cmd"]').value;
                    payload.fast = block.querySelector('select[name="fast"]').value;
                }
...
def run_test_galvo():
    ...
    test_galvo_in_progress = True
    stop_galvo_test_flag['stop_galvo_test'] = False
    script_path_galvo = 'galvo_loop_test_v2.py'              # Path to Galvo test script
    full_output = ""
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    args = [str(device.get('address', '')), mode]
    ...
    if mode == 'manual':
        ...
        args.extend([
            str(device.get('begin', '')),
            str(device.get('end', '')),
            str(device.get('time_up', '')),
            str(device.get('time_down', '')),
            str(device.get('delay', '')),
            str(device.get('cycles', '')),
            str(device.get('cmd', '')),
            str(device.get('fast', ''))
        ])
