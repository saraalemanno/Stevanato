...
# Delta from previous version: send the command AFTER starting the aquisition on the scope (asynchronouse)
# Date: 2025-08-27
# Version: 2
...
def get_device_address():
    if len(sys.argv) >= 3:
        ...
        if test_mode == "manual":
            try:
                angle_req_raw = int(sys.argv[3])
                angle_req = validate_angle_req([angle_req_raw])                                       # Set the bit corresponding to the pin in the output mask
            except ValueError as e:
                print(f"Error: {e}")
                sys.exit(1)

            return device_address, device_namespace, device_name, deviceType, angle_req_raw, test_mode
        elif test_mode == "loop":
            try:
...
def validate_angle_req(angle_req):
    for angle in angle_req:
        if angle < 100 or angle > 65535:
            raise ValueError("Invalid angle. Possible angles (raw position) are between 100 and 65535.")
    return angle_req
...
def run_galvo_test(device_namespace):
    ...
    def galvo_loop_test():
        ...
        digital_in.triggerPositionSet(0) #533
        ...
        if test_mode == "manual":
            ...
            sio.emit('change_mode', change_mode_payload['new_mode'], namespace=device_namespace)
            cycles = command_loop["cycles"]
            time.sleep(1)
            sio.emit('manual_cmd', command_loop, namespace=device_namespace)
            #sio.emit('device_info', namespace=device_namespace)
            start = time.time()
            cycle_values = []
            cycle = 0
            while True:
                ...
                while cycle <= cycles:
                    if sts == DwfState.Done:
                        digital_in.configure(False, True)  
                        sts = digital_in.status(True)
                        cycle += 1
                        print(cycle)
                        print(sts)
...
if __name__ == "__main__":
    ...
    if args[5] == "manual":
        device_address, device_namespace, device_name, deviceType, angle_req_raw, test_mode = args
