...
# Date: 2025-08-07
# Version: 5 to use with gpio_autoloop_test_v3 and I2C_test_v1
...
<body>
    ...
    <button onClick="runI2CTest()">Run I2C Test</button>
    ...
    <script>
        ...
        async function runI2CTest() {
            ...
            try {
                ...
                const result = await response.json();
                document.getElementById('consoleOutput').setAttribute('data-report-path', result.reportPath);
                document.getElementById('consoleOutput').innerHTML += result.output;
                localReportPath = result.reportPath;
            } catch (error) {
                console.error("Error during I2C test:", error);
                alert("Errore durante il test I2C.");
            }
        }
