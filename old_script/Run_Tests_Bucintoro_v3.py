...
if __name__ == "__main__":
    ...
    '''arduinos = {
        addr: ArduinoDevice(port=ser.port, address=addr) 
        for addr, ser in devices.items()
      }
    arduino_main = arduinos.get(0)'''
    if 0 not in arduinos:
        print("[ERROR] Arduino con address 0 NON trovato! Impossibile continuare.")
        sys.exit(1)

    arduino_main = arduinos[0]
    ...
    if err_phase is not None and errors != 0:
        ...
        arduino_main.stop_encoder()
        arduino_main.stop_noise()
        arduino_main.ser.close()
        stop_event.set()
    ...
    if len(sys.argv) < 3:
        ...
        for i, (arduino_addr, arduino) in enumerate(arduinos.items()): 
            ...
            run_galvo_test(galvo_addr,arduino)
            time.sleep(20)
