...
def get_address(ser):
    ...
    response = read_line(ser, "ADDRESS:", timeout=0.5)
    if not response:
        return None
    try:
    ...
    while time.time() < end:
        ...
        if start != -1 and end_json != -1 and end_json > start:
            ...
            try:
                ...
                pos_encoder = obj.get("ENC", None)
                return inputs, pos_encoder
...
def get_angles(ser):
    ...
    encoder_pos = None
    if enc_part and enc_part.startswith("ENC:"):
        try: 
            encoder_pos = int(enc_part.split(":")[1]) 
        except ValueError: 
            encoder_pos = None
